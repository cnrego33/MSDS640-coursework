#=======================================================================
# Baseline Model v2 - Logistic Regression
...
OHE_model_data <- dbGetQuery(con, 'SELECT * FROM "habitat_model_data_v2_OHE"')
dbDisconnect(con)

colnames(OHE_model_data)
nrow(OHE_model_data)
...
OHE_model_data <- OHE_model_data |> select(-locality_id, -n_detections, -n_years)


#make all column names clean and uniform - remove spaces and commas and slashes
#colnames(OHE_model_data) <- make.names(colnames(OHE_model_data))


# fill na with 0 
model_data_cleaned <- OHE_model_data |> 
  mutate(  
...
model_data_cleaned <- model_data_cleaned |> select(-lon, -lat, -seasons)
model_data_cleaned <- model_data_cleaned |> select(-slr_10m, -slr_05m)
model_data_cleaned <- model_data_cleaned |> 
  mutate(nlcd_unknown = as.integer(rowSums(is.na(select(.,starts_with("nlcd_2025_")))) >0)) |>
  mutate(across(starts_with("nlcd_2025_"), ~replace_na(., 0L)))

head(model_data_cleaned)
#=======================================================================
#3. Train Test Split 
...
lr_model_v2 <- glm(presence ~ ., #glm fits generalized linear model, presence ~. predicts presence using all other columns (. is every feature)
                data = train, 
                family = binomial) # binomial designates it as logistic not linear regression

summary(lr_model_v2)

colSums(is.na(model_data_cleaned))
